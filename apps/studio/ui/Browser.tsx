...
export function Browser({ project, integrity, selectedCharId, onSelectChar }: Props) {
  ...
  const ro: [string, unknown[], boolean][] = [
    ...
    ["relationships", project.relationships, project.relationships.some((r) => issueFromIds.has((r as { id: string }).id))],
    ["quests", project.quests, false],
    ["scenes", project.scenes, project.scenes.some((s) => issueFromIds.has((s as { id: string }).id))],
    ["states", project.states, project.states.some((s) => issueFromIds.has((s as { id: string }).id))],
    ["context packages", project.contextPackages, project.contextPackages.some((c) => issueFromIds.has((c as { id: string }).id))],
